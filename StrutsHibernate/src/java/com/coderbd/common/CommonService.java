...
public class CommonService<E> implements CommonDao<E> {
  ...
  //  if(!tr.isActive()) tr.begin();   
    ...
    public List<E> findAll() throws Exception {
        return getSession().createQuery("Select t from " + getEntityClass().getSimpleName() + " t").list();
    }

    @Override
    @Transactional
    public List<E> findByProperty(String prop, Object val) throws Exception {
        return (List<E>) getSession().createQuery("select x from " + getEntityClass().getSimpleName() + " x where x." + prop + " = ?1").setParameter(1, val).list();
    }
